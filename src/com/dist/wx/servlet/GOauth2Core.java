...
import com.dist.wx.api.ParamesAPI;
...
public class GOauth2Core {
	 ...
	 *            微信终端使用此参数判断是否需要带上身份信息 员工点击后，页面将跳转至
	 *            redirect_uri/?code=CODE&state=STATE，企业可根据code参数获得员工的userid
	 */
	public static String GetCode(String redirect_uri) {
		String get_code_url = "";
		get_code_url = GET_CODE.replace("CORPID", ParamesAPI.corpId).replace("REDIRECT_URI",
				WeixinUtil.URLEncoder(redirect_uri));
		return get_code_url;
	}
